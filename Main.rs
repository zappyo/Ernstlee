...
fn main() {
   ...
   // dur: time to sleep for in between actions, default 1 second
   ...
   io::stdin().read_line(&mut _e).ok().expect("This should never happen");
   ...
   let mut player = match &*input.trim().to_lowercase() {
       ...
       "k" | "knight" => {
           ...
           ds::Character::new(
               ...
               "A dastardly thief with high critical hits",


               ds::Weapon::new("Bandit's Knife", 52, 0.50),


               3)


       } _ => panic!("Failed trying to assign a class")
   ...
   for enemy in &mut enemies {
       ...
       println!("\nYou encountered: {} wielding a {}!", enemy.description,
       ...
       if !player.fight(enemy, dur) {
           ...
           println!("Remaining estus: {}", player.flasks);
           ...
           io::stdin().read_line(&mut ans).ok().expect("Failed");
...
fn print_help() {
   ...
   println!("With no OPTION, start game as normal.\n");


   println!("   -f, --fast\tNo downtime between attacks");
   ...
   println!("   text-souls -f\tStart game in sonic mode");


   println!("   text-souls --help\tPrint out this message\n");


   println!("Github: https://github.com/starchery/text-souls");
